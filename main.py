...
generation = model.generate(idx, new_tokens_count=300)[0].tolist()
print(decode(generation))

B, T, C = 4, 8, 2
xrand = torch.randn(B, T, C)
tril = torch.tril(torch.ones(T, T))
weights = torch.zeros(T, T)
masked_fill = weights.masked_fill(tril == 0, float('-inf'))
weights = masked_fill.softmax(dim=1)
xbow = weights @ xrand
